...
import type { Data, PokemonSet } from "@pkmn/sets";
...
const FORMATS = [
  ...
  "gen7randombattle",
  "gen8randombattle",
  /* 'gen7randomdoublesbattle', */ "gen8randomdoublesbattle",
  "gen9randombattle",
...
const p1spec = FIXED
  ? { name: "Bot A", team: DTeams.pack(importTeam(TEAM_A, "A")) }
  : { name: "Bot A" };
const p2spec = FIXED
  ? { name: "Bot B", team: DTeams.pack(importTeam(TEAM_B, "B")) }
  : { name: "Bot B" };
  ...
  "|faint|"(args: Protocol.Args["|faint|"]) {
    ...
    if (poke) {
      ...
      for (const child in $td.children) {
        if (
          ($td.children[child] as HTMLImageElement).dataset.name === poke.name
        ) {
