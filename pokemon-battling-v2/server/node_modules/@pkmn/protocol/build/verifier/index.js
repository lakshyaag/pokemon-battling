...
var _chunk7I5PBXEZjs = require('../chunk-7I5PBXEZ.js');
...
var Verifier = class {
  ...
  verify(data) {
    for (const { roomid, args, kwArgs } of _chunk7I5PBXEZjs.Protocol.parse(data)) {
  ...
  verifyLine(line) {
    const parsed = _chunk7I5PBXEZjs.Protocol.parseBattleLine(line);
  ...
  dispatch(args, kwArgs, raw = false) {
    ...
    const key = _chunk7I5PBXEZjs.Protocol.key(args, raw);
    if (!key || !handler[key]) return false;
    if (Object.keys(kwArgs).length && !(key in _chunk7I5PBXEZjs.Protocol.ARGS_WITH_KWARGS)) return false;
